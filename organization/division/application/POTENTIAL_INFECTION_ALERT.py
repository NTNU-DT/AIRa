...
def run_r0_alert_app():
    steps = st.sidebar.slider(label='Number of CO2 real-time data items', min_value=1, max_value=30, value=20, format="%d",step=1)
    ...
    print(f)
    x_i=pd.read_csv('../co2.csv').tail(steps).iloc[:, 1]
    time_i=np.array(pd.read_csv('../co2.csv').tail(steps).iloc[:, 0])
