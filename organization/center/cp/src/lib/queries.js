...
class QueryUtils {
    ...
    * @param {String} alertNumber  commercial paper number
    */
    async getAssetHistory(issuer, alertNumber ) {

        let ledgerKey = await this.ctx.stub.createCompositeKey(this.name, [issuer, alertNumber ]);
    ...
    */
    ...
    // This is an example of a parameterized query accepting a single query parameter 
    ...
    */
    async queryKeyByEndorser(endorser) {
        //  
        let self = this;
        if (arguments.length < 1) {
            throw new Error('Incorrect number of arguments. Expecting endorser name.');
        }
        let queryString = {};
        queryString.selector = {};
        //  queryString.selector.docType = 'indexOwnerDoc';
        queryString.selector.endorser = endorser;
    ...
    // ===== Example: Parameterized rich query =================================================
    // queryKeyByIssuer queries for assets based on a passed in owner.
    // This is an example of a parameterized query accepting a single query parameter 
    // Only available on state databases that support rich query (e.g. CouchDB)
    // =========================================================================================
    /**
    * queryKeyByIssuer commercial paper
    * @param {String} owner commercial paper owner
    */
    async queryKeyByIssuer(issuer) {
        //  
        let self = this;
        if (arguments.length < 1) {
            throw new Error('Incorrect number of arguments. Expecting endorser name.');
                        ...
                        try {
                            ...
                            switch (jsonRes.Value.currentState) {
                                ...
                                case 3:
                                    ...
                                    break;
                                case 4:
                                    jsonRes.Value.currentState = 'REFUTED';
                                    break;
